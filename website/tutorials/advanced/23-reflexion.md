...
description: Build agents that learn from failures with episodic memory - implementing verbal reinforcement learning for iterative problem-solving
---
...
Test your understanding of the Reflexion pattern:

<Quiz
  question="What is the fundamental difference between Reflection and Reflexion patterns?"
  tutorial-id="23-reflexion"
  :options="[
    { text: 'Reflexion is always faster than Reflection', correct: false },
    { text: 'Reflection refines one draft iteratively, while Reflexion makes multiple fresh attempts with episodic memory of ALL past attempts', correct: true },
    { text: 'Reflection requires external tools while Reflexion does not', correct: false },
    { text: 'They are identical patterns with different spelling', correct: false }
  ]"
  explanation="Reflection refines a SINGLE draft through an iterative generate-critique-revise loop. Reflexion makes MULTIPLE fresh attempts and maintains EPISODIC MEMORY of all past attempts and reflections, enabling the agent to learn from failures across iterations rather than just improving one output."
  :hints="[
    { text: 'Think about what happens to information from previous attempts', penalty: 10 },
    { text: 'The comparison table shows Memory: Single output vs All attempts (episodic)', penalty: 15 }
  ]"
/>

<Quiz
  question="What is the purpose of operator.add in the Reflexion state schema for attempts and reflections?"
  tutorial-id="23-reflexion"
  :options="[
    { text: 'To perform arithmetic addition on numerical values', correct: false },
    { text: 'To accumulate attempts and reflections across iterations, creating episodic memory', correct: true },
    { text: 'To increment the iteration counter automatically', correct: false },
    { text: 'To concatenate search result strings', correct: false }
  ]"
  explanation="Using Annotated[list[dict], operator.add] for attempts and Annotated[list[str], operator.add] for reflections enables automatic list concatenation. Each iteration appends new entries, preserving the complete history. This creates episodic memory where all past attempts inform future iterations."
  :hints="[
    { text: 'Look at the ReflexionState TypedDict definition', penalty: 10 },
    { text: 'operator.add with lists causes concatenation, building up history over time', penalty: 15 }
  ]"
/>

<Quiz
  question="What are the three main phases in each Reflexion iteration?"
  tutorial-id="23-reflexion"
  :options="[
    { text: 'Think, Act, Observe (ReAct pattern)', correct: false },
    { text: 'Draft (answer + search queries), Execute tools (gather information), Revise (incorporate findings)', correct: true },
    { text: 'Plan, Execute, Evaluate (Plan-and-Execute)', correct: false },
    { text: 'Generate, Critique, Approve (Reflection pattern)', correct: false }
  ]"
  explanation="Each Reflexion iteration has three phases: 1) DRAFT - generate an answer with self-reflection and propose search queries to fill knowledge gaps, 2) EXECUTE - run the search queries to gather external information, and 3) REVISE - update the answer based on search results and add citations/references."
  :hints="[
    { text: 'Look at the architecture diagram showing the flow through nodes', penalty: 10 },
    { text: 'The phases involve answering, searching, then improving based on search results', penalty: 15 }
  ]"
/>

<Quiz
  question="What does the Reflection structured output model capture in Reflexion?"
  tutorial-id="23-reflexion"
  :options="[
    { text: 'Only a numerical score from 1-10', correct: false },
    { text: 'The missing information and superfluous information in the current answer', correct: true },
    { text: 'Just the final revised answer', correct: false },
    { text: 'Only the search queries to execute', correct: false }
  ]"
  explanation="The Reflection Pydantic model captures two key aspects: 1) 'missing' - what critical information is lacking in the answer, and 2) 'superfluous' - what unnecessary information should be removed. This structured self-critique guides focused improvements in subsequent iterations."
  :hints="[
    { text: 'Look at the Reflection BaseModel definition with its fields', penalty: 10 },
    { text: 'The model helps identify what to add and what to remove', penalty: 15 }
  ]"
/>

<Quiz
  question="When should you use Reflexion instead of simple Reflection?"
  tutorial-id="23-reflexion"
  :options="[
    { text: 'When you need to refine a single document draft', correct: false },
    { text: 'When writing professional emails', correct: false },
    { text: 'For research questions requiring external information gathering and learning from past failures', correct: true },
    { text: 'When speed is the top priority', correct: false }
  ]"
  explanation="Use Reflexion when: 1) Research questions need external information from search tools, 2) The agent lacks knowledge and needs to gather facts, 3) You want to learn from multiple attempts rather than just refine one, and 4) Fact-checking or verification is needed. Reflection is better for content refinement tasks."
  :hints="[
    { text: 'Consider what makes Reflexion unique: external search and episodic memory', penalty: 10 },
    { text: 'The When to Use section lists research questions, knowledge gaps, and fact-checking', penalty: 15 }
  ]"
/>

<Quiz
  question="How many search queries should be generated per Reflexion iteration?"
  tutorial-id="23-reflexion"
  type="fill-blank"
  :accepted-answers="['1-3', '1 to 3', 'one to three', '1', '2', '3']"
  explanation="The tutorial recommends limiting search queries to 1-3 per iteration. This prevents information overload while still allowing targeted information gathering. The AnswerQuestion model enforces this with max_length=3 on the search_queries field."
  :hints="[
    { text: 'Look at the AnswerQuestion Pydantic model definition', penalty: 10 },
    { text: 'The Best Practices section mentions preventing information overload', penalty: 15 }
  ]"
/>
...
<div class="tutorial-nav">
  <a href="/tutorials/advanced/22-reflection" class="prev">← Tutorial 22: Reflection</a>
  <a href="/tutorials/advanced/24-lats" class="next">Tutorial 24: LATS →</a>
</div>
